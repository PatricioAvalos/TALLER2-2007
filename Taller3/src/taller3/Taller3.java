...
public class Taller3 {
    ...
    public static void LecturaTxtClientes(ListaClientes listaClientes) throws IOException{
    ...
    public static void LecturaTxtLibros(ListaLibros listaLibros) throws IOException{
    ...
    public static void LecturaTxtPrestamos(ListaPrestamos listaPrestamos) throws IOException{
    ...
    // RF1
    public static void RF1(ListaClientes ListaC, ListaLibros ListaL, 
            ListaPrestamos ListaP)throws ParseException{
        ...
        StdOut.println(ListaP.getCantidadPrestamos());
        //int [] ArrayDeudas = new int[ListaP.getCantidadPrestamos()];
        StdOut.println("Cliente: "+ListaC.encontrarCliente(RutIngresado).getNombre()+" | RUT: "+RutIngresado);
        for(i=0;i<ListaP.getCantidadPrestamos();i++){
            if(RutIngresado.equals(ListaP.getPrestamoI(i).getRUT())){
                int CodigoPrestamo = ListaP.getPrestamoI(i).getNumReg();
                Date FechaPrestamo = StringADate(ListaP.getPrestamoI(i).getFechaPedido());
                Date FechaDevoEstimada = StringADate(ListaP.getPrestamoI(i).getFechaDevolucion());
                
                if(ListaP.getPrestamoI(i).getFechaDevuelto().equals("0")){
                int Deuda = 0;
                DeudaTotal +=Deuda;
                ...
                Date FechaDevoReal = StringADate(ListaP.getPrestamoI(i).getFechaDevuelto());
    ...
    // RF2
    public static void RF2(ListaLibros ListaL, 
            ListaPrestamos ListaP)throws ParseException{
        
        int i,j;
        String [] ArrayAuxISBNLibro = new String[ListaL.getCantidadLibros()];
        int [] ArrayAuxCant = new int[ListaL.getCantidadLibros()];
        for (i = 0; i < ListaL.getCantidadLibros(); i++) {
            ArrayAuxISBNLibro[i] = ListaL.getLibroI(i).getISBN();
            ArrayAuxCant[i] = 0;
        }
        ...
        for (i = 0; i < ListaL.getCantidadLibros(); i++) {
            for (j = 0; j < ListaP.getCantidadPrestamos(); j++) {
                Date AuxFecha = StringADate(ListaP.getPrestamoI(j).getFechaPedido());
                if (ComparadorRangoFechas(AuxFecha, FInicial, FFinal)==true) {
                    if(ArrayAuxISBNLibro[i].equals(ListaP.getPrestamoI(j).getISBN())){
                    ArrayAuxCant[i] += 1;
                    }
                }   
            }
    ...
    //RF3
    public static void RF3 (ListaClientes ListaC, ListaLibros ListaL, 
            ListaPrestamos ListaP)throws ParseException{
        int i,j;
        int cantzero=0;
        int indice = 0;
        ...
        for(i=0;i<ListaP.getCantidadPrestamos();i++){
            if(ListaP.getPrestamoI(i).getFechaDevuelto().equals("0")){
                RUTs[indice] = ListaP.getPrestamoI(i).getRUT();
                CodUnicoPrestamo[indice] = ListaP.getPrestamoI(i).getNumReg();
                FechaDevEstimada[indice] = StringADate(ListaP.getPrestamoI(i).getFechaDevolucion());
                indice++;
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        LecturaTxtClientes(listaClientes);
        
        ListaPrestamos listaPrestamos = new ListaPrestamos(1000);
        LecturaTxtPrestamos(listaPrestamos);
        
        ListaLibros listaLibros = new ListaLibros(1000);
        LecturaTxtLibros(listaLibros);
        
        
        
        try {
            RF3(listaClientes, listaLibros, 
            listaPrestamos);
        } catch (Exception e) {
        }
        ...
        RF1(listaClientes, listaLibros, 
            listaPrestamos);
        RF2(listaLibros, listaPrestamos);
        try {
        Date X = StringADate("14-03-1994");
        Date a = StringADate("13-03-1994");
        Date b = StringADate("20-03-1994");
        System.out.println(ComparadorRangoFechas(X,a,b));
            System.out.println(DiferenciaDiasDeFechas(a, b));
        } catch (Exception e) {
        }
        **/
